...
- Classic bikes that must be docked at a station to end a ride.
- Electric bikes that can be locked to a public bike rack within the service area _or_ docked at a station.
...
So what's the goal of this data analysis? Simple: We want to uncover how annual riders and non-members use Divvy bikes differently.
...
>
> The resulting CSV file came in at a hefty 1GB and has over 5.6 millions records. That's a ton of rides sift thought! I then uploaded it into Google Cloud Storage and created a table named `2022_tripdata` on BigQuery based on the CSV file.

### Cleaning strategy
First thing first, we will identify what should be cleaned based on our business goal: how annual riders and non-members use Divvy bikes differently.

In the second part, we will actually clean the dataset with all the points we will have then defined in the previous part.


### Duplicates and `NULL` values
First thing first, let's check if there are no duplicate `ride_id` and if the `rideable_type`, `started_at`, `ended_at` and `member_casual` fields contain `NULL` values.
...
I noticed that a lot of fields have either the `start_station_name` or `end_station_name` missing, and sometimes both (!). 

This could lead to some issues down the road when we'll analyze trip data by station. So let's run the following query to check how many records have missing station data:
...
That's a lot of `NULL` data! It also shows that when station data is missing, the station name and station ID fields are `NULL`. Moving forward we will use the station name fields instead of the station id fields.
...
I reached out to Lyft's data service and they told me changes were made to the Divvy platform in April 2020. Before that date, a classic bike was described as `docked_bike`. Since April 2020, the same bikes are now tracked as `classic_bike`.

That's good to know, but I guess the implementation is not perfect yet as there are still some `docked_bike` in the 2022 dataset. We will have to replace all instances of `docked_bike` value with `classic_bike`.
...
There are also quite some trips above 3 hours. We might have to do some cleaning there since the bikes were probably lost or stolen.

So let's dig further and see how the distribution of ride durations looks, starting with members:
...
We will dig deeper into this theme in the analysis part. As a reminder, casual riders with a day pass can use a classic bike for up to 3 hours. Like members, we'll give them an extra 30 minutes widows and remove every record from casual riders where the trip duration was above 3 hours and 30 minutes.

### Trips beginning and ending in the same station
It is common to begin and end a trip at the same station when using a bike-sharing service. There are plenty of reasons to do so. It could be because of a false start, a broken bike, or grocery shopping. Or even just for a quick errand. 
...
But the most interesting bit is with members. The median is actually quite low: 2 minutes and 11 seconds. This suggests that they may have unlocked a bike and then returned it because it was defective.
...
Let's start by exporting the list of all stations using the following query:
```sql
...
Once the data is loaded into our Google Sheets, we can now start using the `VLOOKUP` formula to look up all the coordinates data from the City of Chicago dataset to the list of stations we previously exported.

In the face of overwhelming odds, we're left with only 1 option: we're going to have to sh*t out of this.


After uploading the dataset on Google Sheets, I encountered errors while looking up the latidute using `=VLOOKUP` formula. Upon inspection, it appears that the name of some stations have changed between the 2 dataset. For instance, `Fullerton Ave & Narragansett Ave` became `Public Rack - Fullerton Ave & Narragansett Ave`.
...
We still got some errors, though. Time to fire up custom formulas using Google App Script and its [Geocoder Class](https://developers.google.com/apps-script/reference/maps/geocoder#reverseGeocode(Number,Number)) to get the latitude and longitude of an address according to Google Maps.
...
Then we can update the formula to get the latitude and longitude *IF* the first 2 VLOOKUP return an empty field:
```less
